'''
指数平滑法认为时间序列的态势具有稳定性或规则性，所以时间序列可被合理地顺势推延；他认为最近的过去态势，在某种程度上
会持续到最近的未来，所以将较大的权数放在最近的资料．
'''
#指数平滑算法
...
def compute_double(alpha, s):
    ...
    s_double = compute_double(alpha,s_single)
    ...
    for i in range(len(s)):
        ...
        #计算二次指数平滑的b
        b_double[i] = (alpha/(a-alpha))*(s_single[i]-s_double[i])
...
def compute_triple(alpha, s):
    ...
    s_single = compute_single(alpha,s)
    s_double = compute_double(alpha,s_single)
    s_triple = compute_triple(alpha,s_double)
...
if __name__ == '__main__':
    ...
    s_single = compute_single(alpha,data)
    print(alpha*data[-1]+(1-alpha)*s_single[-1])