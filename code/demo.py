'''
指数平滑法是生产预测中常用的一种方法，也用于中短期经济发展趋势预测，所有预测方法中，指数平滑是用的最多的一种．简单的
全期平均法是对时间数列的过去数据一个不漏地全部加以利用；移动平均法则不考虑较远期的数据，并在加权移动平均法中给予近期
资料更大的权重；而指数平滑法则兼容了全期平均和移动平均所长，不舍弃过去的数据，但是仅给逐渐减弱的影响程度，即随着数据
远离，赋予逐渐收敛为零的权数．
alpha越大，新数据所占的比重就越大，原预测值所占比重就越小．
一次指数平滑预测：y1 = alpha * y' + (1-alpha) * y''
二次指数平滑预测：y2 = alpha * y1 + (1-alpha) * y2'
三次指数平滑预测：y3 = alpha * y2 + (1-alpha) * y3'
'''
import numpy as np
...
def show_data(new_year, pre_year, data, s_pre_double, s_pre_triple):
    ...
    plt.xticks(new_year) #设置ｘ轴的刻度线为new_year
    plt.show()

def main():
    alpha = .70 #设置alpha,即平滑系数
    pre_year = np.array([2016, 2017]) #将需要预测的两年存入numpy的array对象里
    data_path = r'../data/data1.txt' #设置数据路径
    data = np.loadtxt(data_path)
    print(data)
    year, time_id, number = data.T #将数据分别复制给year,time_id,number
    initial_line = np.array([0,0,number[0]])  #初始化，由于平滑指数是根据上一期的数值进行预测的，原始数据中最早数据１９９４，这里94和95划为相同
    print(initial_line)
    ...
    #计算二次平滑次数，二次平滑是在一次平滑的基础上进行的，三次平滑是在二次平滑的基础上进行的
    s_double = exponential_smoothing(alpha, s_single)

    a_double = 2*s_single-s_double #计算二次指数平滑的ａ
    ...
    for i in range(1,len(initial_time_id)):
        #循环计算每一年的二次指数平滑法的预测值，下面三次指数平滑原理相同
